...
/datum/category_item/player_setup_item/background/culture/sanitize_character()
	...
	// This is a bit noodly. If pref.cultural_info[TAG_CITIZENSHIP] is null, then we haven't finished loading/sanitizing, which means we might purge
	...
	if(pref.cultural_info[TAG_CITIZENSHIP])
		var/decl/cultural_info/check = GET_DECL(pref.cultural_info[TAG_CITIZENSHIP])
		if(check)
